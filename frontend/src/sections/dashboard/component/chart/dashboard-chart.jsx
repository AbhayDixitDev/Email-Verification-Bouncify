...
export function DashboardChart({ title, subheader, showAlert, chart, handleAlertClose, details, onDownload, forceShowChart, ...other }) {
  ...
  // Chart related setup
  const showChart = forceShowChart !== undefined ? forceShowChart : (((!isUploading && !isUploaded && !isStartVerification && !isVerificationCompleted) || isVerificationCompleted));
  const showProgressLinear = forceShowChart ? false : (isUploading || (isStartVerification && !isVerificationCompleted));
  const showChartAlert = forceShowChart ? false : (!isUploading && isUploaded && !isStartVerification && !isVerificationCompleted);
          ...
          {showChart && (
            <>
              ...
              <ChartLegends
                ...
                values={chartSeries}
                totalEmails={chartSeries.reduce((a, b) => a + b, 0)}
                sx={{
                  py: 2,
